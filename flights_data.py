...
QUERY_PCT_DELAYED_FLIGHTS_BY_ROUTE = """
  ...
  FROM (
    SELECT
      ...
      COUNT(*) AS 'TOTAL_FLIGHTS',
      SUM(CASE WHEN DEPARTURE_DELAY >= 20 THEN 1 ELSE 0 END) AS 'DELAYED_FLIGHTS',
	  origins.LATITUDE AS 'O_LAT',
	  origins.LONGITUDE AS 'O_LON',
	  destinations.LATITUDE AS 'D_LAT',
	  destinations.LONGITUDE AS 'D_LON'
    FROM flights
	  JOIN airports AS origins ON flights.ORIGIN_AIRPORT = origins.IATA_CODE
	  JOIN airports AS destinations ON flights.DESTINATION_AIRPORT = destinations.IATA_CODE
